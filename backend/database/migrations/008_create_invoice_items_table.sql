...
-- Description: Stores line items for invoices
-- Created: 2024-01-25
...
COMMENT ON COLUMN invoice_items.item_id IS 'Reference to item master (nullable for custom items)';
COMMENT ON COLUMN invoice_items.name IS 'Item name (snapshot at time of creation)';
COMMENT ON COLUMN invoice_items.line_total IS 'Calculated line total: (quantity * price * (1 - discount/100)) * (1 + tax/100)';
